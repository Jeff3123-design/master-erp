...
export const Purchases: React.FC = () => {
  const { purchases, setPurchases, suppliers, products, updatePurchaseStatus, addAuditLog, showToast } = useApp();
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    addAuditLog('PURCHASE_CREATED', 'Purchasing', newPO.poNumber, null, newPO);
  ...
  const handleReceivePO = (poId: string) => {
    ...
    addAuditLog('PURCHASE_RECEIVED', 'Purchasing', po.poNumber, { status: po.status }, { status: 'RECEIVED' });
    showToast(`PO ${po.poNumber} marked as RECEIVED. Stock inventory automatically updated!`, 'success');
  };
                ...
                <td className="py-3 px-4">
                  <span
                    className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${
                      po.status === 'RECEIVED'
                        ? 'bg-blue-100 text-blue-700 dark:bg-navy-800 dark:text-brand-400'
                        : po.status === 'APPROVED'
                ...
                <td className="py-3 px-4 text-right space-x-2">
                  ...
                  {po.status === 'APPROVED' && (
                    <button
                      ...
                      className="px-3 py-1 bg-brand-600 hover:bg-brand-700 text-white text-xs font-bold rounded-lg shadow-xs flex items-center space-x-1 ml-auto"
                    >
